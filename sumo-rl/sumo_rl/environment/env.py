...
class SumoEnvironment(gym.Env):
    ...
    def _compute_step_info(self):
        ...
        res = {
			...
			'total_average_speed': sum(self.traffic_signals[ts].get_average_speed() for ts in self.ts_ids),
			'total_pressure': sum(self.traffic_signals[ts].get_pressure() for ts in self.ts_ids),
			'lane_density': sum(sum(self.traffic_signals[ts].get_lanes_density()) for ts in self.ts_ids),
			'out_lane_density': sum(sum(self.traffic_signals[ts].get_out_lanes_density()) for ts in self.ts_ids)
        }
        print(res)
        return res
